...
class PriorityQueue:
    ...
    def add(self, node):
        ...
        self.data.sort(key=self.f,reverse=True) # Sort by f(node) = path_cost
...
# basically build empty adjacency matrix
#this list comprehension will make 14 X14 matrix for me and initialize all the  values as 0
adj = [[0 for _ in range(n)] for _ in range(n)]


#how have i defined this?
# index_node_1 -> index_node_2, the direct travelling cost
edges_clean = [
...
# Build adjacency matrix from edges_clean
...
class Problem:
    ...
    def ACTIONS(self, state):
        ...
        for i in range(n):
            if adj[state][i] != 0:
                actions.append(i)
        return actions  # List of the directly next city indices
